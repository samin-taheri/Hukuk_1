...
export default function AccountActivity() {
    ...
    const [courtOfficeUpdateId, setCourtOfficeUpdatId] = useState(0);
    ...
    const getAllAccountActivities = () => {
        if (authService.DoesHaveMandatoryClaim('TransactionActivitySubTypeGetAll')) {
    ...
    const addNewRecord = () => {
        let obj = {
            TransactionAcitivitySubTypeName: profileName,
            isActive: statusForAdd
        }
        let re
        if (courtOfficeUpdateId > 0) {
            obj.TransactionAcitivitySubTypeId = courtOfficeUpdateId
    ...
    function openModelForUpdate(id) {
        accountActivityService.getById(id).then(result => {
            if (result.data.Success) {
                ...
                setProfileName(caseTypeUpdate.TransactionAcitivitySubTypeName)
                setStatusForAdd(caseTypeUpdate.IsActive)
                setCourtOfficeUpdatId(caseTypeUpdate.TransactionAcitivitySubTypeId)
            }
        })
        setOpen(true)
    }
    return (
        <Page title="Account Activity | MediLaw">
            <Container>
                <Stack direction="row" alignItems="center" justifyContent="space-between" mb={4}>
                    <Typography variant="h4" gutterBottom>
                        Account Activity
                    </Typography>
                    {authService.DoesHaveMandatoryClaim('TransactionActivitySubTypeAdd') ? (
                        <>
                                ...
                                {accountActivities.length > 0 ? (
                                    <TableContainer component={Paper}>
                                        <Table sx={{minWidth: 650}} aria-label="simple table">
                                            <TableHead>
                                                <TableRow>
                                                    <TableCell sx={{paddingLeft: 7}}>Transaction Activity Name</TableCell>
